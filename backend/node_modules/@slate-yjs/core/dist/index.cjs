...
function slatePointToRelativePosition(sharedRoot, slateRoot, point) {
  ...
  return Y6.createRelativePositionFromTypeIndex(yParent, textRange.start + point.offset, point.offset === textRange.end ? -1 : 0);
}
...
function mergeNode(sharedRoot, slateRoot, op) {
  ...
  if (!prev.yTarget || !target.yTarget) {
    ...
    const previousSibling = import_slate11.Node.get(slateRoot, import_slate11.Path.previous(op.path));
    if (!import_slate11.Text.isText(previousSibling)) {
...
var CursorEditor = {
  isCursorEditor(value) {
    return YjsEditor.isYjsEditor(value) && value.awareness instanceof import_awareness.Awareness && typeof value.cursorDataField === "string" && typeof value.selectionStateField === "string" && typeof value.sendCursorPosition === "function" && typeof value.sendCursorData === "function";
  },
